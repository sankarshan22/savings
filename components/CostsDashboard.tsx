...
const CostsDashboard: React.FC<CostsDashboardProps> = ({ onBack, bills, members }) => {
  ...
  const sortedDates = useMemo(() => {
    return Object.keys(costsByDate).sort((a, b) => {
                  ...
                  return (
                      <div key={date} className="bg-[#121212]/50 rounded-lg p-4 transition-all duration-300 hover:bg-[#2E2E2E]/50">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
                          <div>
                            <span className="text-xs font-medium text-[#FF6B81] bg-[#FF6B81]/10 px-3 py-1 rounded-full inline-block mb-1">
                              📅 {date}
                            </span>
                            <p className="font-semibold text-[#F2F2F2] mt-1">{formatDate(date)}</p>
                          </div>
                          <span className="font-mono font-semibold text-xl text-[#FF6B81]">{formatCurrency(dailyCost)}</span>
                        </div>
                        <div>

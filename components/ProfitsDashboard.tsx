...
const ProfitsDashboard: React.FC<ProfitsDashboardProps> = ({ onBack, bills, members }) => {
  ...
  const sortedDates = useMemo(() => {
    return Object.keys(profitsByDate).sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
  }, [profitsByDate]);
                  ...
                  return (
                      <div key={date} className="bg-[#121212]/50 rounded-lg p-4 transition-all duration-300 hover:bg-[#2E2E2E]/50">
                        <div className="flex justify-between items-center mb-3">
                          ...
                          <span className="font-mono font-semibold text-[#A8E6CF]">{formatCurrency(dailyProfit)}</span>
                        </div>
                        <div>
